...
- **Worker** (`src/agent.py`): `_PrewarmingFishTTS` implements the base-class `prewarm()` hook (the framework calls it at agent-activity start, right before the greeting synthesis) with a throwaway HEAD to api.fish.audio through the plugin's own aiohttp session, so the first utterance skips DNS+TCP+TLS. The design build starts at the top of `my_agent`; the preset greeting `generate_reply` still runs before `ctx.connect()`; `preemptive_generation` is on for preset sessions.
